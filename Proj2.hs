...
*    Purpose:        Code for Project 2, Semester 2, 2021.
*
*    Battleship Implementations.
*    This implementation takes a "generate and test" approach;
*    every time feedback is recorded all the possible positions are calculated
...
-- | feedback returns a tuple for the distances that are zero,
--   one and two units away from a target.
--   Example:
...
--   that the three targets could be hiding and filtering
--   the places that isn't in that list.
--   Example:
--   > nextGuess ([(1, 2), (3, 4), (5, 6)],
--                [[(1, 2), (3, 4), (5, 6)]]) (3, 0, 0)
--     ([(1,2),(3,4),(5,6)],[[(1,2),(3,4),(5,6)]])
nextGuess ::
     ([Location], GameState) -> (Int, Int, Int) -> ([Location], GameState)
nextGuess (guess, state) results = (head newState, newState)
  where
